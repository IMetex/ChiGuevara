...
namespace Mete.Scripts
{
    public class RangeEnemyController : MonoBehaviour
    {
        ...
        {
            cooldownTimer = 0;
            enemyBullet[EnemyBullets()].transform.position = bulletExit.position;
            enemyBullet[EnemyBullets()].GetComponent<EnemyProjectile>().ActivateProjectile();
        }

        private int EnemyBullets()
        {
            for (int i = 0; i < enemyBullet.Length; i++)
            {
                if (!enemyBullet[i].activeInHierarchy)
                    return i;
            }
